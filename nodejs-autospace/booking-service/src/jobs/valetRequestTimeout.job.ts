...
export function startValetRequestTimeoutJob() {
  cron.schedule("* * * * *", async () => {
    try {
      ...
      logger.info(
        `[ValetTimeout] ${stale.length} stale valet request(s) found`,
      );
      ...
      for (const booking of stale) {
        ...
        try {
          logger.warn(
            `[ValetTimeout] Timed out: bookingId=${booking.id} valetId=${timedOutValetId}`,
          );
          ...
          logger.info(
            `[ValetTimeout] Reassigned: bookingId=${booking.id} (from valetId=${timedOutValetId})`,
          );
        } catch (err) {
          logger.error(
            `[ValetTimeout] Failed to timeout booking ${booking.id}:`,
            err,
          );
        }
      }
