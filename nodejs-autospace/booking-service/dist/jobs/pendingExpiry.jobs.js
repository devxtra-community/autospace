    ...
    cron.schedule("* * * * *", async () => {
        try {
            ...
            if (!expiredBookings.length)
                return;
            logger.info(`Expiring ${expiredBookings.length} payment_pending bookings`);
            for (const booking of expiredBookings) {
                try {
                    ...
                    if (payment?.stripePaymentIntentId) {
                        try {
                            ...
                            if (intent.status === "succeeded") {
                                logger.warn(`Webhook missed — auto confirming booking ${booking.id}`);
                    ...
                    await bookingRepo.save(booking);
                    await axios.post(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${booking.slotId}/release`, {}, {
                        headers: {
...
//# sourceMappingURL=pendingExpiry.jobs.js.map