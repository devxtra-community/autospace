...
export class BookingService {
    async checkOverlap(slotId, start, end) {
    ...
    async lockSlot(slotId) {
        const response = await axios.post(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${slotId}/lock`, {}, {
            headers: {
    ...
    async releaseSlot(slotId) {
        await axios.post(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${slotId}/release`, {}, {
            headers: {
                Authorization: `Bearer ${process.env.INTERNAL_SERVICE_TOKEN}`,
                "x-user-id": "booking-service",
                "x-user-role": "SERVICE",
    ...
    async releaseValet(valetId) {
        await axios.patch(`${process.env.RESOURCE_SERVICE_URL}/internal/valets/${valetId}/release`, {}, {
            headers: {
                Authorization: `Bearer ${process.env.INTERNAL_SERVICE_TOKEN}`,
                "x-user-id": "booking-service",
                "x-user-role": "SERVICE",
    ...
    async createBooking(bookingData) {
        ...
        if (bookingData.valetRequested) {
            const garageRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/internal/garages/${bookingData.garageId}`, {
                headers: {
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
                },
            ...
            const distance = calculateDistanceKm(Number(garage.latitude), Number(garage.longitude), bookingData.pickupLatitude, bookingData.pickupLongitude);
            if (distance > garage.valetServiceRadius) {
                throw new ValidationError(`Pickup outside valet radius (${garage.valetServiceRadius} km)`);
            }
        }
        const idempotencyKey = `booking:idemp:${bookingData.userId}:${bookingData.slotId}:${bookingData.startTime.toISOString()}`;
        const idempotencyResult = await redisClient.set(idempotencyKey, "processing", { NX: true, EX: 60 });
        ...
        try {
            // Fetch slot details
            const slotResponse = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${bookingData.slotId}`, {
                headers: {
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
                },
            ...
            const userRes = await axios.get(`${process.env.AUTH_SERVICE_URL}/internal/users/${savedBooking.userId}`, {
                headers: {
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
                },
            ...
            const slotRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${savedBooking.slotId}`, {
                headers: {
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
                },
...
You may access your parking slot directly using your secure Entry PIN at the scheduled time.`;
            await sendMail(user.email, "Your Autospace Booking is Confirmed", `Dear ${user.fullname},
...
`);
    ...
    async assignFirstValetRequest(booking, manager) {
        try {
            const response = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/internal/valets/available/${booking.garageId}`, {
                headers: {
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
                    "x-user-email": "service@internal",
    ...
    async rejectValetRequest(bookingId, valetId) {
        const bookingRepo = AppDataSource.getRepository(Booking);
    ...
    async updateBookingWithValet(booking) {
        const bookingRepo = AppDataSource.getRepository(Booking);
    ...
    async getManualAssignments(managerId) {
        const bookingRepo = AppDataSource.getRepository(Booking);
        const garageRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/internal/garages/manager/${managerId}`, {
            headers: {
                "x-user-id": managerId,
                "x-user-role": "SERVICE",
                "x-user-email": "service@internal",
        ...
        let allAvailableValets = [];
        try {
            const valetsRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/valets/all-active/${garageId}`, {
                headers: {
                    "x-user-id": managerId,
                    "x-user-role": "SERVICE",
                    "x-user-email": "service@internal",
            ...
            logger.error("Failed to fetch active valets for manual assignment", { garageId, error: err instanceof Error ? err.message : err });
        }
        const enriched = await Promise.all(bookings.map(async (booking) => {
            ...
            try {
                ...
                const userRes = await axios.get(`${process.env.AUTH_SERVICE_URL}/internal/users/${booking.userId}`, { headers });
                ...
                const slotRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${booking.slotId}`, { headers });
                ...
                logger.error("Failed to enrich booking for manual assignment", { bookingId: booking.id, error: err instanceof Error ? err.message : err });
            }
            const rejectedIds = booking.rejectedValetIds || [];
            const availableValets = allAvailableValets.filter((v) => v.employmentStatus === "ACTIVE" && !rejectedIds.includes(v.id));
            return {
        ...
        try {
            ...
            if (cached) {
                ...
                if (Array.isArray(parsed)) {
                    const invalidCache = parsed.some((b) => typeof b !== "object" || b === null || !("slot" in b));
                    if (!invalidCache) {
                        logger.info("User bookings retrieved from Redis", { userId });
                        return parsed;
                    }
                    logger.info("User bookings cache outdated — refreshing from DB", { userId });
                }
            }
            const bookings = await bookingRepo.find({
                where: { userId },
            ...
            const enriched = await Promise.all(bookings.map(async (booking) => {
                ...
                if (booking.valetId) {
                    try {
                        ...
                        if (axios.isAxiosError(err)) {
                            ...
                            logger.error("Valet fetch error during enrichment", { valetId: booking.valetId, message: err.message });
                        }
                        else {
                            logger.error("Unknown valet error during enrichment", { valetId: booking.valetId, error: err });
                        }
                    }
                }
                /* ================= SLOT ================= */
                if (booking.slotId) {
                    try {
                        const slotRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/internal/slots/${booking.slotId}`, {
                            headers: {
                                "x-user-id": "booking-service",
                                "x-user-role": "SERVICE",
                            },
                        ...
                        if (axios.isAxiosError(err)) {
                            ...
                            logger.error("Slot fetch error during enrichment", { slotId: booking.slotId, message: err.message });
                        }
                        else {
                            logger.error("Unknown slot error during enrichment", { slotId: booking.slotId, error: err });
                        }
                    }
                }
                return {
            ...
            if (err instanceof Error) {
                logger.error("getUserBookings failed", { userId, message: err.message });
            }
            else {
    ...
    async updateBookingStatus(bookingId, status) {
        ...
        if (booking.status !== "payment_pending" && status === "confirmed") {
            logger.info("Skip status update — booking already finalized", { bookingId: booking.id });
            return booking;
        }
        booking.status = status;
        if (status === "occupied") {
            await axios.post(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${booking.slotId}/occupy`, {}, {
                headers: {
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
                },
        ...
        if (status === "completed") {
            ...
            await axios.post(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${booking.slotId}/free`, {}, {
                headers: {
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
                },
    ...
    async enrichBookings(bookings) {
        const enrichedBookings = await Promise.all(bookings.map(async (booking) => {
            try {
                ...
                const userRes = await axios.get(`${process.env.AUTH_SERVICE_URL}/internal/users/${booking.userId}`, { headers });
                const garageRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/internal/garages/${booking.garageId}`, { headers });
                const slotRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/garages/internal/slots/${booking.slotId}`, { headers });
                const user = userRes.data.data;
                const garage = garageRes.data.data;
                const slot = slotRes.data.data;
                ...
                logger.error("Booking enrichment failed", { bookingId: booking.id, error: err instanceof Error ? err.message : err });
                return null;
            }
        }));
        return enrichedBookings.filter(Boolean);
    }
    async getValetRequests(valetId) {
        const bookingRepo = AppDataSource.getRepository(Booking);
        const bookings = await bookingRepo.find({
            where: {
        ...
        logger.info("Valet requests retrieved from database", { valetId, count: bookings.length });
        return await this.enrichBookings(bookings);
    }
    async getActiveJobs(valetId) {
        const bookingRepo = AppDataSource.getRepository(Booking);
        const bookings = await bookingRepo.find({
            where: {
                valetId,
    ...
    async getCompletedJobs(valetId) {
        const bookingRepo = AppDataSource.getRepository(Booking);
        const bookings = await bookingRepo.find({
            where: {
                valetId,
    ...
    async sendValetStatusEmail(booking) {
        const headers = {
            "x-user-id": "booking-service",
            "x-user-role": "SERVICE",
        };
        ...
        const garage = garageRes.data.data;
        // fetch slot
        const slotRes = await axios.get(`${process.env.RESOURCE_SERVICE_URL}/internal/slots/${booking.slotId}`, { headers });
        const slot = slotRes.data.data;
        // format dates
        ...
        await sendMail(user.email, `Autospace Update: ${statusTitle}`, `
Dear ${user.fullname},
...
`);
    ...
    async updateValetStatus(bookingId, valetStatus) {
        const bookingRepo = AppDataSource.getRepository(Booking);
        const booking = await bookingRepo.findOne({
            where: { id: bookingId },
        });
    ...
    async verifyPickupPin(bookingId, valetId, pin) {
        const bookingRepo = AppDataSource.getRepository(Booking);
        ...
        if (booking.pickupPinUsed)
            throw new Error("Pickup PIN already verified");
        if (!booking.pickupPin || booking.pickupPin !== pin)
            throw new Error("Invalid pickup PIN");
        if (booking.valetStatus !== BookingValetStatus.ASSIGNED &&
