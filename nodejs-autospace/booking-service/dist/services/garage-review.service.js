...
export const submitGarageReviewService = async (userId, bookingId, rating, comment) => {
...
export const getGarageReviewsService = async (garageId, limit = 3, offset = 0) => {
    ...
    const enriched = await Promise.all(reviews.map(async (review) => {
        try {
            const res = await axios.get(`${process.env.AUTH_SERVICE_URL}/internal/users/${review.userId}`, {
                headers: {
...
export const getGarageAverageRating = async (garageId) => {
    const repo = AppDataSource.getRepository(GarageReview);
    const reviews = await repo.find({
        where: { garageId },
    });
...
//# sourceMappingURL=garage-review.service.js.map