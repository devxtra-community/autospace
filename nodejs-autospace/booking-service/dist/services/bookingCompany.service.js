...
export const getCompanyBookingsService = async (companyId, filters) => {
    const bookingRepo = AppDataSource.getRepository(Booking);
    const page = Number(filters.page) || 1;
    const limit = Number(filters.limit) || 10;
    const skip = (page - 1) * limit;
    const garageRes = await axios.get(`${RESOURCE_SERVICE_URL}/garages/byCompany/${companyId}`, {
        headers: {
    ...
    const users = await Promise.all(bookings.map(async (b) => {
        try {
            const res = await axios.get(`${AUTH_SERVICE_URL}/internal/users/${b.userId}`, {
                headers: {
                    Authorization: `Bearer ${process.env.INTERNAL_SERVICE_TOKEN}`,
                    "x-user-id": "booking-service",
                    "x-user-role": "SERVICE",
    ...
    if (filters.search) {
        ...
        data = data.filter((d) => d.customerName.toLowerCase().includes(s) ||
...
//# sourceMappingURL=bookingCompany.service.js.map