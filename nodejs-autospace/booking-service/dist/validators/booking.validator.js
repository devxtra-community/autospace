...
export function validateId(id, field) {
...
export function validateBookingInput(data) {
    const { slotId, userId, garageId, startTime, endTime, status, pickupAddress, pickupLatitude, pickupLongitude, valetRequested, } = data;
    if (slotId)
        validateId(slotId, "slotId");
    if (userId)
        validateId(userId, "userId");
    if (garageId)
        validateId(garageId, "garageId");
    ...
    if (valetRequested) {
        if (pickupLatitude === undefined ||
            ...
            !pickupAddress) {
            throw new ValidationError("Pickup location required when valet is requested");
        }
    }
    // Status validation
    if (status) {
        ...
        if (!validStatuses.includes(status)) {
            throw new ValidationError(`Invalid status. Allowed: ${validStatuses.join(", ")}`);
        }
    }
}
